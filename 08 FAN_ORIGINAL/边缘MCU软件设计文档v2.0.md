...
graph TD
    ...
    subgraph 初始化
        ...
        I2 --> I3[设置PWM=0%低电平]
        I3 --> I4[启动TIM3 PWM输出]
        I5[初始化FG捕获变量<br/>fg_capture_count=0<br/>fg_frequency=0<br/>fg_last_time=0<br/>fg_current_time=0]
        I4 --> I6[初始化SPI从机通信]
    end
    
    %% === 主循环 ===
    subgraph 主循环
        M1[等待接收SPI命令] --> M2{解析命令}
        
        M2 -->|xPWMxxx| M3[x:0-3对应PUMP/FAN1/2/3<br/>xxx:关断百分比off%]
        M3 --> M4[duty = 100 - off%]
        M4 --> M5[调用PWM_SetDutyPercent_ById<br/>设置对应通道PWM占空比]
        M5 --> M1
        
        M2 -->|FGRDxxxx| M7[解析EN0/EN1/ADDR0/1/2]
        M7 --> M8[切换MUX选择FG通道]
        M8 --> M9[重置FG捕获统计]
        M9 --> M10[等待20ms模拟开关稳定settle_until = start_tick + 20ms]
        M10 --> M11[启动单次FG捕获]
        M11 --> M1
        
        M2 -->|FGRDxxxx| M12[读取当前fg_frequency]
        M12 --> M13[RPM = 60 * fFG / 2]
        M13 --> M14[通过SPI回传转速值至主MCU]
        M14 --> M1
    end

    %% === TIM2输入捕获中断ISR ===
    subgraph TIM2_CH2输入捕获中断ISR
        T1[FG边沿触发中断] --> T2[读取CCR2捕获值]
        T2 --> T3{fg_capture_count==0?}
        T3 -- 是 --> T4[记录首次捕获值<br/>fg_last_time=CCR2<br/>fg_current_time=CCR2]
        T4 --> T5[fg_capture_count++]
        
        T3 -- 否 --> T6[fg_last_time=fg_current_time]
        T6 --> T7[fg_current_time=CCR2]
        T7 --> T8{fg_current_time > fg_last_time?}
        T8 -- 是 --> T9[fg_period = fg_current_time - fg_last_time]
        T8 -- 否 --> T10[处理计数器溢出]
        T9 --> T11[计算频率fFG = TIM2_CLK / PSC / fg_period]
        T10 --> T11
        T11 --> T12[fg_capture_count++]
        T5 --> T13[更新last_fg_update_time]
        T12 --> T13
        T13 --> T14[退出中断]
    end

    %% === FG单次捕获处理（主循环中轮询）=== 
    subgraph FG单次捕获处理主循环轮询
        F1{fg_one_shot_active?} -- 否 --> F10[退出]
        F1 -- 是 --> F2{当前时间 >= settle_until?}
        F2 -- 否 --> F9[等待1ms后<br/>继续轮询]
        F9 --> F1
        F2 -- 是 --> F3{fg_capture_count >= 2?}
        F3 -- 是 --> F4[计算RPM = 30 * fg_frequency]
        F4 --> F5[通过UART打印结果]
        F5 --> F6[fg_one_shot_active=0]
        F6 --> F10
        F3 -- 否 --> F7{超时>2000ms?}
        F7 -- 是 --> F8[打印超时错误<br/>freq_to_report=0]
        F8 --> F6
        F7 -- 否 --> F9
    end
...
边缘MCU通过SPI接收主MCU的指令，指令格式与现有UART指令保持一致：

| 指令 | 格式 | 说明 | 示例 |
|------|------|------|------|
| **PWM设置** | `xPWMxxx` | x: 0-3对应PUMP/FAN1/FAN2/FAN3<br>xxx: 关断百分比(000-100)<br>实际占空比 = 100 - off% | `0PWM000` → PUMP 0%占空比<br>`1PWM050` → FAN1 50%占空比<br>`2PWM100` → FAN2 100%占空比 |
| **FG通道切换** | `FGRDxxxx` | 第1位x: EN选择(0→EN0=0,EN1=1; 1→EN0=1,EN1=0)<br>第2-4位: ADDR0/ADDR1/ADDR2 | `FGRD0100` → EN0=0,EN1=1,ADDR=000<br>`FGRD1001` → EN0=1,EN1=0,ADDR=001 |
| **转速读取** | `FG_RD` | 立即捕获当前FG信号并回传转速 | 回传格式: `RPM:xxxx` |

#### 3.1.2 FG转速计算公式

```
fFG = TIM2_CLK / (PSC + 1) / fg_period
RPM = 60 × fFG / 2 = 30 × fFG
```
...
- TIM2_CLK = 72MHz (系统时钟)
- PSC = 71 (预分频值，实际分频72)
- fg_period: 两次捕获之间的时间差(TIM2计数器值)

#### 3.1.3 关键全局变量

| 变量名 | 类型 | 说明 |
|--------|------|------|
| `fg_capture_count` | uint32_t | FG边沿捕获计数 |
| `fg_frequency` | float | 计算得到的FG频率(Hz) |
| `fg_last_time` | uint32_t | 上次捕获时间(TIM2计数器值) |
| `fg_current_time` | uint32_t | 当前捕获时间(TIM2计数器值) |
| `fg_period` | uint32_t | 两次捕获周期(TIM2计数器差值) |
| `fg_one_shot_active` | uint8_t | 单次FG捕获激活标志 |
| `fg_one_shot_settle_until` | uint32_t | MUX稳定等待截止时间(ms) |
| `fg_one_shot_timeout_ms` | uint32_t | 单次捕获超时时间(2000ms) |
| `last_fg_update_time` | uint32_t | 上次FG更新时刻(ms) |

#### 3.1.4 中断资源分析

**中断触发机制：**
- TIM2_CH2配置为输入捕获模式，FG信号每个边沿触发一次中断
- 典型风扇转速1000-6000 RPM对应FG频率约16.7-100Hz
- 中断间隔 = 1/fFG，最小约10ms（6000 RPM时）

**中断执行时间分析：**
```
中断处理内容：
1. 读取CCR2寄存器: ~1-2个时钟周期
2. 变量赋值和比较: ~10-20个时钟周期  
3. 浮点除法(计算频率): ~100-200个时钟周期
4. 总计: ~200-300个时钟周期 @72MHz ≈ 3-4μs
```

**资源占用评估：**
- **CPU占用率**: 100Hz时约0.04%，完全可接受
- **RAM占用**: 静态变量约32字节
- **中断嵌套**: 中断内无阻塞操作，可安全嵌套

**settle_until说明：**
- 在收到`FGRDxxxx`命令时设置：`settle_until = HAL_GetTick() + 20ms`
- 目的：等待CD4051 MUX模拟开关切换后信号稳定
- 20ms经验值：覆盖MUX切换时间(约10μs) + 信号建立时间
