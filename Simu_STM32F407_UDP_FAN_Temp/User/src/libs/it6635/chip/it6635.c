...
#pragma diag_suppress 68  //关闭warning:  #68-D: integer conversion resulted in a change of sign
#pragma diag_suppress 188 //关闭warning:  #188-D: enumerated type mixed with another type
#pragma diag_suppress 224 //关闭warning:  #224-D: the format string requires additional arguments
...
u8 h2swwr(u8 offset, u8 wdata)
{
    ...
    return it6635_i2c_write(gdev.opts.dev_opt->SwAddr, offset, 1, &wdata);
}
...
u8 h2swrd(u8 offset)
{
    ...
    it6635_i2c_read(gdev.opts.dev_opt->SwAddr, offset, 1, &rddata);
...
void h2swbrd(u8 offset, u8 length, u8 *rddata)
{
    if ( length > 0 )
    {
        it6635_i2c_read(gdev.opts.dev_opt->SwAddr, offset, length, rddata);
    }
}
...
void h2swbwr(u8 offset, u8 length, u8 *rddata)
{
    ...
    if ( length > 0 )
    {
        it6635_i2c_write(gdev.opts.dev_opt->SwAddr, offset, length, rddata);
    }
}
...
u8 h2rxedidwr(u8 offset, u8 *wrdata, u8 length)
{
    ...
    return it6635_i2c_write(gdev.opts.dev_opt->EdidAddr, offset, length, wrdata);
}

u8 h2rxedidrd(u8 offset, u8 *wrdata, u8 length)
{
    return it6635_i2c_read(gdev.opts.dev_opt->EdidAddr, offset, length, wrdata);
}
...
u8 h2rxwr(u8 offset, u8 rdata)
{
    ...
    return it6635_i2c_write(gdev.opts.dev_opt->RxAddr, offset, 1, &rdata);
}
...
u8 h2rxrd(u8 offset)
{
    ...
    it6635_i2c_read(gdev.opts.dev_opt->RxAddr, offset, 1, &rddata);
...
void h2rxbrd(u8 offset, u8 length, u8 *rddata)
{
    if ( length > 0 )
    {
        it6635_i2c_read(gdev.opts.dev_opt->RxAddr, offset, length, rddata);
    }
}
...
void h2rxbwr(u8 offset, u8 length, u8 *rddata)
{
    ...
    if ( length > 0 )
    {
        it6635_i2c_write(gdev.opts.dev_opt->RxAddr, offset, length, rddata);
    }
}
...
u8 cecwr(u8 offset, u8 wdata)
{
    ...
    return it6635_i2c_write(gdev.opts.dev_opt->CecAddr, offset, 1, &wdata);
}
...
u8 cecrd(u8 offset)
{
    ...
    it6635_i2c_read(gdev.opts.dev_opt->CecAddr, offset, 1, &rddata);
...
void cecbrd(u8 offset, u8 length, u8 *rddata)
{
    if ( length > 0 )
    {
        it6635_i2c_read(gdev.opts.dev_opt->CecAddr, offset, length, rddata);
    }
}
...
void cecbwr(u8 offset, u8 length, u8 *rddata)
{
    ...
    if ( length > 0 )
    {
        it6635_i2c_write(gdev.opts.dev_opt->CecAddr, offset, length, rddata);
    }
}
...
void _sw_init(void)
{
    ...
    h2swwr(0xEF, gdev.opts.dev_opt->RxAddr | 0x01);
    ...
    if ( gdev.opts.EnCEC )
    {
        ...
        h2swwr(0xEE, (gdev.opts.dev_opt->CecAddr | 0x01));
    ...
    #endif
    {
		...
		tmp = 0x40;
		it6635_i2c_write(gdev.opts.dev_opt->CecAddr, 0x10, 1, &tmp); // Disable CEC_IOPU
        h2swwr(0xEE, (gdev.opts.dev_opt->CecAddr & 0xFE));
    }
    ...
    {
        h2swwr(0xd9 + port * 2, 0x00);    // Port0 VSDB_AB
        h2swwr(0xda + port * 2, 0xFF);    // Port0 VSDB_CD
    }
}
...
void _sw_reset(void)
{
    ...
    if(h2swrd(0xEF) == (gdev.opts.dev_opt->RxAddr | 0x01))
    {
...
#endif
{
    ...
    switch ( gdev.vars.state_sys_fsm )
    {
        ...
        case RX_PORT_CHANGE:
            ...
            // the following code is moved to it6635_fsm()
            // to avoid race condition
            #if (PORT_CHANGE_WITH_FSM == 0)
            ...
            if (gdev.opts.active_rx_opt->NonActivePortReplyHPD == 0)
            {
    ...
    switch ( gdev.vars.state_sys_fsm )
    {
        ...
        case TX_OUTPUT:
            ...
            //if ( gdev.opts.WaitSymlockBeforeTXOE )
            {
                if ( 0 == _rx_is_all_ch_symlock() )
                {
                    if ( 0 == _rx_is_any_ch_symlock() )
                    {
                        ...
                        if ( gdev.vars.count_symlock_lost == 100 )
                        {
							_rx_pll_reset();
                            it6635_toggle_hpd(1000);
                            //it6635_set_tx_5v(0);
            ...
            if ( gdev.opts.active_rx_opt->FixIncorrectHdmiEnc )
            {
                ...
                if ( gdev.vars.current_hdmi_mode != HDMI_MODE_AUTO && gdev.opts.active_rx_opt->EnRxDDCBypass == false )
                {
...
void it6635_irq(void)
{
...

    {
        ...
        if ( currBD & 0xe0 ) {
            ...
            PR_INFO(("---HDCP BD=%02x (%lu)\r\n", currBD, (u32)(gdev.vars.tick_hdcp-gdev.vars.tick_set_afe)));
            //prevBD = currBD;
            _sw_clear_hdcp_status();
...
void it6635_setup_edid_phy_addr(u8 vsdb_pos, u8 port, u16 phyaddr)
{
    u8 phyAB, phyCD;
    u8 sum;
    ...
    if ( vsdb_pos )
    {
        h2swwr(0xe9, vsdb_pos);
        h2rxwr(0x4B, gdev.opts.dev_opt->EdidAddr | 0x01);
        h2rxset(0x4C, 0x0F, (1<<port));

        PR_INFO(("vsdb_pos=%02x\r\n", vsdb_pos));

        phyAB = (u8)(phyaddr>>8)&0xFF;
        phyCD = (u8)(phyaddr)&0xFF;
        sum = (0x100 - gdev.vars.EdidChkSum[1] - phyAB - phyCD) & 0xFF;

        h2swwr(0xd9 + port * 2, phyAB);
        h2swwr(0xda + port * 2, phyCD);
        h2swwr(0xe2 + port * 2, (u8)sum);
        h2rxedidwr(vsdb_pos, &phyAB, 1);
        h2rxedidwr(vsdb_pos+1, &phyCD, 1);
        h2rxedidwr(128+127, &sum, 1);

        h2rxwr(0x4B, gdev.opts.dev_opt->EdidAddr);
    }
}
#endif
...
__RETRY_EDID_READ:
    ...
    {
        u8 *def_edid = gdev.vars.default_edid[flag];
...
bool it6635_setup_edid_ram(u8 flag)
{
    ...
    {
        edid_tmp[0x7E] = 1;
        edid_tmp[0x7F] = _rx_calc_edid_sum(edid_tmp);
        gdev.vars.EdidChkSum[0] = edid_tmp[0x7F];
    }
    #endif
    ...
    for ( i = 1; i <= extblock; i++ )
    {
        ...
        if ( i==1 ) // assume our sink has only 2 block EDID
        {
            ...
            if ( gdev.opts.tx_opt->ParsePhysicalAddr )
            {
                ...
                {
                    #if EDID_PA_MANUAL_ASSIGN
                    it6635_setup_edid_phy_addr(gdev.vars.VSDBOffset, 0, PA_PORT0);
                    it6635_setup_edid_phy_addr(gdev.vars.VSDBOffset, 1, PA_PORT1);
                    it6635_setup_edid_phy_addr(gdev.vars.VSDBOffset, 2, PA_PORT2);
                    #else
...
void it6635_dump_register_all(void)
{
    ...
    // dump EDID RAM
    h2rxwr(0x4B, gdev.opts.dev_opt->EdidAddr | 0x01); // enable EDID RAM i2c address
    ...
    it6635_dump_register(gdev.opts.dev_opt->EdidAddr, "\n*** EDID Port 2:\n");
    h2rxwr(0x4B, gdev.opts.dev_opt->EdidAddr); // disable EDID RAM i2c address
...
void it6635_set_internal_EDID( u8 block, u8 *edid, u8 target_port )
{
    int i;
    u8 wcount = 16;
