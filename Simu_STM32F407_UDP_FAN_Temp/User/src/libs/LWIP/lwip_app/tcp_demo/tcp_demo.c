...
static u8_t connectFlag = 0;
u16_t TCP_Server_PORT=8080;//todo:菜单设置
u16_t TCP_Client_PORT=8080;//从accept相关的信息中获取；
char TCP_recvbuf[256];	//
...
{                 
	...
	tcp_bind(tcp_server_pcb,IP_ADDR_ANY,TCP_Server_PORT);  //绑定IP和端口号
...
static err_t OnTcpServerRecv(void *arg, struct tcp_pcb *pcb,struct pbuf *p,err_t err)
{
	if(p != NULL)    //p是数据缓冲区指针，不为空表示有数据到
	{
		...
		memset(TCP_recvbuf,0,50);  
    memcpy(TCP_recvbuf,p->payload, p->tot_len);
		printf("TCP_recvbuf=%s\r\n",TCP_recvbuf);
		TCP_data_len=0x3F;
		pbuf_free(p); 								  //释放缓冲区
	}
	return ERR_OK;
...
static void OnTcpReceiveData(void *recvData, unsigned short len)
{
	//Add code here
	//在此处添加接收到数据的处理代码
	SendTcpDataToClient(recvData, len);//test:回传数据
...
void CheckForConnection(void)
{
	...
	{
		if((cpcb->local_port == TCP_Server_PORT)&&(tcp_client_pcb->state!=CLOSED))  //如果TCP_LOCAL_PORT端口指定的连接没有断开
		{
			connectFlag = 1;  						//连接标志
			break;							   	
		}
