...
{
	uint8_t rval=0;
	ETH_GPIO_InitTypeDef GPIO_InitStructure;
	...
	if(0 == rval)
		printf("RJ45 LAN not connect. PHY init fail.\n");
    if (ETH_ReadPHYRegister(LAN8720_PHY_ADDRESS, PHY_BSR) & 4) // PHY_SR//LEO 20181031
    {
...
uint8_t ETH_MACDMA_Config(void)
{
    ...
    while (ETH_GetSoftwareResetStatus() == SET);//等待
	...
	// ETH_InitStructure.ETH_BroadcastFramesReception = ETH_BroadcastFramesReception_Disable;//允许接收所有广播帧
	...
	if(EthInitStatus==ETH_SUCCESS)//配置成功, reset正常，link状态正常，自协商正常
	{
		ETH_DMAITConfig(ETH_DMA_IT_NIS|ETH_DMA_IT_R,ENABLE);  	//使能以太网接收中断	
		printf("ETH_DMAITConfig\n");
	}
...
void ETH_IRQHandler(void)
{
	...
	while(ETH_GetRxPktSize(DMARxDescToGet)!=0) 	//检测是否收到数据包 if// 20230922 while(ETH_CheckFrameReceived())//
	{ 
		lwip_pkt_handle();		
	}
...
//  uint8_t iptxt[20];
  ...
  if(netif_is_link_up(netif))
  {
	...
	// ETH_Init(&ETH_InitStructure, LAN8720_PHY_ADDRESS);
...
void ETH_Broadcast_Set(int flag)
{
	...
	if(flag==0)
		tmpreg |= ETH_BroadcastFramesReception_Disable;
	else
		tmpreg |= ETH_BroadcastFramesReception_Enable;
