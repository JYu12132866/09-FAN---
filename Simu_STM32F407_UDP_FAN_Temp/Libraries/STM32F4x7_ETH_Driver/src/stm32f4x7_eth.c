...
__IO ETH_DMADESCTypeDef  *DMATxDescToSet={0};//

__IO ETH_DMADESCTypeDef  *DMARxDescToGet={0};//

/* Structure used to hold the last received packet descriptors info */

 ETH_DMA_Rx_Frame_infos RX_Frame_Descriptor={0};

__IO ETH_DMA_Rx_Frame_infos *DMA_RX_FRAME_infos={0};//
...
if(ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
  {
		    timeout = 0;
		 ETH_WritePHYRegister(PHYAddress, 16, 0x031B);        //RMII  50M,CRS_DV
		    do
    {
      timeout++;
    ...
    {
		//	HAL_UART_Transmit(&UartHandle, (uint8_t *)"timeout1\n ", 9, 5000);
      return ETH_ERROR;
    }
    ...
    timeout = 0;
		...
		{
	//		HAL_UART_Transmit(&UartHandle, (uint8_t *)"timeout2\n ", 9, 5000);
			/* Return ERROR in case of write timeout */
      return ETH_ERROR;
     ...
     if(timeout == PHY_READ_TO)
     {
	//		 HAL_UART_Transmit(&UartHandle, (uint8_t *)"timeout3\n ", 9, 5000);
      return ETH_ERROR;
      }
 /* Reset Timeout counter */
    timeout = 0;
     RegValue = ETH_ReadPHYRegister(PHYAddress, ANLPAR);
		...
		{
//			HAL_UART_Transmit(&UartHandle, (uint8_t *)"ETH_full\n ", 9, 5000);
			ETH_InitStruct->ETH_Mode = ETH_Mode_FullDuplex; 
		}
		else
		{
	//		HAL_UART_Transmit(&UartHandle, (uint8_t *)"ETH_half\n ", 9, 5000);
			ETH_InitStruct->ETH_Mode = ETH_Mode_HalfDuplex; 
			
		}
		 if((RegValue & PHY_ANLPAR_100TX)||(RegValue & PHY_ANLPAR_100TXFD))
		 {
//			 HAL_UART_Transmit(&UartHandle, (uint8_t *)"ETH_100\n ", 9, 5000);
			   ETH_InitStruct->ETH_Speed = ETH_Speed_100M; 
		 }
		else
		{
//			HAL_UART_Transmit(&UartHandle, (uint8_t *)"ETH_10\n ", 9, 5000);
...
//DMATxDescToSet->Buffer1Addr=0;
//DMATxDescToSet->Status=0;
//DMATxDescToSet->ControlBufferSize=0;
//DMATxDescToSet->Buffer2NextDescAddr=0;

//DMARxDescToGet->Buffer1Addr=0;
//DMARxDescToGet->Status=0;
//DMARxDescToGet->ControlBufferSize=0;
//DMARxDescToGet->Buffer2NextDescAddr=0;


//DMA_RX_FRAME_infos->FS_Rx_Desc=NULL;
//DMA_RX_FRAME_infos->LS_Rx_Desc=NULL;
//DMA_RX_FRAME_infos->Seg_Count=0;
//(DMA_RX_FRAME_infos->FS_Rx_Desc)->Buffer1Addr=0;
//(DMA_RX_FRAME_infos->FS_Rx_Desc)->Status=0;
//(DMA_RX_FRAME_infos->FS_Rx_Desc)->ControlBufferSize=0;
//(DMA_RX_FRAME_infos->FS_Rx_Desc)->Buffer2NextDescAddr=0;
    /* Reset Timeout counter */
    timeout = 0;
    /* Read the result of the auto-negotiation */
    RegValue = ETH_ReadPHYRegister(PHYAddress,PHY_BSR );//PHY_SR LEO 20181031
...
{ 
 ...
 __IO FrameTypeDef frame ; 
	frame.length=0;
	frame.descriptor=NULL;
	frame.buffer=0;
  (frame.descriptor)->Buffer1Addr=0;
	  (frame.descriptor)->Buffer2NextDescAddr=0;
	  (frame.descriptor)->ControlBufferSize=0;
	  (frame.descriptor)->Status=0;
...
/**
  ...
  * @retval error status
  */
uint32_t ETH_Prepare_Transmit_Descriptors(uint32_t FrameLength)
{   
