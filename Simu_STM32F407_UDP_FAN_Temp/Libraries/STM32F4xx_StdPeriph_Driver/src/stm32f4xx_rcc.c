...
#define HSION_ETH_BitNumber           0x00
#define CR_ETH_HSION_BB               (PERIPH_BB_BASE + (CR_OFFSET * 32) + (HSION_ETH_BitNumber * 4))
/* Alias word address of CSSON bit */
#define CSSON_ETH_BitNumber           0x13
#define CR_ETH_CSSON_BB               (PERIPH_BB_BASE + (CR_OFFSET * 32) + (CSSON_ETH_BitNumber * 4))
/* Alias word address of PLLON bit */
#define PLLON_ETH_BitNumber           0x18
#define CR_ETH_PLLON_BB               (PERIPH_BB_BASE + (CR_OFFSET * 32) + (PLLON_ETH_BitNumber * 4))
/* Alias word address of PLLI2SON bit */
#define PLLI2SON_ETH_BitNumber        0x1A
#define CR_ETH_PLLI2SON_BB            (PERIPH_BB_BASE + (CR_OFFSET * 32) + (PLLI2SON_ETH_BitNumber * 4))

/* --- CFGR Register ---*/
/* Alias word address of I2SSRC bit */
#define CFGR_OFFSET               (RCC_OFFSET + 0x08)
#define I2SSRC_ETH_BitNumber          0x17
#define CFGR_ETH_I2SSRC_BB            (PERIPH_BB_BASE + (CFGR_OFFSET * 32) + (I2SSRC_ETH_BitNumber * 4))

/* --- BDCR Register ---*/
/* Alias word address of RTCEN bit */
#define BDCR_OFFSET               (RCC_OFFSET + 0x70)
#define RTCEN_ETH_BitNumber           0x0F
#define BDCR_ETH_RTCEN_BB             (PERIPH_BB_BASE + (BDCR_OFFSET * 32) + (RTCEN_ETH_BitNumber * 4))
/* Alias word address of BDRST bit */
#define BDRST_ETH_BitNumber           0x10
#define BDCR_ETH_BDRST_BB             (PERIPH_BB_BASE + (BDCR_OFFSET * 32) + (BDRST_ETH_BitNumber * 4))

/* --- CSR Register ---*/
/* Alias word address of LSION bit */
#define CSR_OFFSET                (RCC_OFFSET + 0x74)
#define LSION_ETH_BitNumber           0x00
#define CSR_ETH_LSION_BB              (PERIPH_BB_BASE + (CSR_OFFSET * 32) + (LSION_ETH_BitNumber * 4))

/* --- DCKCFGR Register ---*/
/* Alias word address of TIMPRE bit */
#define DCKCFGR_OFFSET            (RCC_OFFSET + 0x8C)
#define TIMPRE_EHT_BitNumber          0x18
#define DCKCFGR_ETH_TIMPRE_BB         (PERIPH_BB_BASE + (DCKCFGR_OFFSET * 32) + (TIMPRE_EHT_BitNumber * 4))
...
#define CIR_ETH_BYTE2_ADDRESS         ((uint32_t)(RCC_BASE + 0x0C + 0x01))
...
void RCC_HSEConfig(uint8_t RCC_HSE)
{
  ...
  *(__IO uint8_t *) CR_BYTE3_ADDRESS = RCC_ETH_HSE_OFF;
...
void RCC_AdjustHSICalibrationValue(uint8_t HSICalibrationValue)
{
  uint32_t tmpreg = 0;
  /* Check the parameters */
  assert_param(IS_RCC_ETH_CALIBRATION_VALUE(HSICalibrationValue));
...
void RCC_HSICmd(FunctionalState NewState)
{
  ...
  *(__IO uint32_t *) CR_ETH_HSION_BB = (uint32_t)NewState;
}
...
{
  /* Check the parameters */
  assert_param(IS_RCC_ETH_LSE(RCC_LSE));

  /* Reset LSEON and LSEBYP bits before configuring the LSE ------------------*/
  /* Reset LSEON bit */
  *(__IO uint8_t *) BDCR_ADDRESS = RCC_EHT_LSE_OFF;
  ...
  *(__IO uint8_t *) BDCR_ADDRESS = RCC_EHT_LSE_OFF;
  ...
  switch (RCC_LSE)
  {
    case RCC_EHT_LSE_ON:
      /* Set LSEON bit */
      *(__IO uint8_t *) BDCR_ADDRESS = RCC_EHT_LSE_ON;
      break;
    case RCC_EHT_LSE_Bypass:
      /* Set LSEBYP and LSEON bits */
      *(__IO uint8_t *) BDCR_ADDRESS = RCC_EHT_LSE_Bypass | RCC_EHT_LSE_ON;
      break;
    default:
      break;
  }
...
void RCC_LSICmd(FunctionalState NewState)
{
  ...
  *(__IO uint32_t *) CSR_ETH_LSION_BB = (uint32_t)NewState;
}
...
void RCC_PLLConfig(uint32_t RCC_PLLSource, uint32_t PLLM, uint32_t PLLN, uint32_t PLLP, uint32_t PLLQ)
{
  ...
  assert_param(IS_RCC_ETH_PLLM_VALUE(PLLM));
  assert_param(IS_RCC_ETH_PLLN_VALUE(PLLN));
  assert_param(IS_RCC_ETH_PLLP_VALUE(PLLP));
  assert_param(IS_RCC_ETH_PLLQ_VALUE(PLLQ));
...
void RCC_PLLCmd(FunctionalState NewState)
{
  /* Check the parameters */
  assert_param(IS_FUNCTIONAL_STATE(NewState));
  *(__IO uint32_t *) CR_ETH_PLLON_BB = (uint32_t)NewState;
}
...
{
  /* Check the parameters */
  assert_param(IS_RCC_ETH_PLLI2SN_VALUE(PLLI2SN));
  assert_param(IS_RCC_ETH_PLLI2SR_VALUE(PLLI2SR));
...
void RCC_PLLI2SCmd(FunctionalState NewState)
{
  /* Check the parameters */
  assert_param(IS_FUNCTIONAL_STATE(NewState));
  *(__IO uint32_t *) CR_ETH_PLLI2SON_BB = (uint32_t)NewState;
}
...
void RCC_ClockSecuritySystemCmd(FunctionalState NewState)
{
  /* Check the parameters */
  assert_param(IS_FUNCTIONAL_STATE(NewState));
  *(__IO uint32_t *) CR_ETH_CSSON_BB = (uint32_t)NewState;
}
...
void RCC_MCO1Config(uint32_t RCC_MCO1Source, uint32_t RCC_MCO1Div)
{
  ...
  /* Check the parameters */
  assert_param(IS_RCC_ETH_MCO1SOURCE(RCC_MCO1Source));
...
void RCC_MCO2Config(uint32_t RCC_MCO2Source, uint32_t RCC_MCO2Div)
{
  ...
  /* Check the parameters */
  assert_param(IS_RCC_ETH_MCO2SOURCE(RCC_MCO2Source));
...
void RCC_HCLKConfig(uint32_t RCC_SYSCLK)
{
  ...
  /* Check the parameters */
  assert_param(IS_ETH_RCC_HCLK(RCC_SYSCLK));
...
void RCC_PCLK1Config(uint32_t RCC_HCLK)
{
  ...
  /* Check the parameters */
  assert_param(IS_ETH_RCC_PCLK(RCC_HCLK));
...
void RCC_RTCCLKConfig(uint32_t RCC_RTCCLKSource)
{
  ...
  /* Check the parameters */
  assert_param(IS_RCC_ETH_RTCCLK_SOURCE(RCC_RTCCLKSource));
...
void RCC_RTCCLKCmd(FunctionalState NewState)
{
  ...
  *(__IO uint32_t *) BDCR_ETH_RTCEN_BB = (uint32_t)NewState;
}
...
void RCC_BackupResetCmd(FunctionalState NewState)
{
  /* Check the parameters */
  assert_param(IS_FUNCTIONAL_STATE(NewState));
  *(__IO uint32_t *) BDCR_ETH_BDRST_BB = (uint32_t)NewState;
}
...
void RCC_I2SCLKConfig(uint32_t RCC_I2SCLKSource)
{
  ...
  *(__IO uint32_t *) CFGR_ETH_I2SSRC_BB = RCC_I2SCLKSource;
}
...
void RCC_TIMCLKPresConfig(uint32_t RCC_TIMCLKPrescaler)
{
  ...
  *(__IO uint32_t *) DCKCFGR_ETH_TIMPRE_BB = RCC_TIMCLKPrescaler;
...
void RCC_ITConfig(uint8_t RCC_IT, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    *(__IO uint8_t *) CIR_ETH_BYTE2_ADDRESS |= RCC_IT;
  }
  else
  {
    /* Perform Byte access to RCC_CIR[14:8] bits to disable the selected interrupts */
    *(__IO uint8_t *) CIR_ETH_BYTE2_ADDRESS &= (uint8_t)~RCC_IT;
  }
}
