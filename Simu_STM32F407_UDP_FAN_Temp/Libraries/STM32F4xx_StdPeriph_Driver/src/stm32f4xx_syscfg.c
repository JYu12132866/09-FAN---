...
#define CMP_ETH_PD_BitNumber          ((uint8_t)0x00) 
#define CMPCR_ETH_CMP_PD_BB           (PERIPH_BB_BASE + (CMPCR_OFFSET * 32) + (CMP_ETH_PD_BitNumber * 4)) 
...
void SYSCFG_CompensationCellCmd(FunctionalState NewState)
{
  ...
  *(__IO uint32_t *) CMPCR_ETH_CMP_PD_BB = (uint32_t)NewState;
}
